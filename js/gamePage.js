...
// Parâmetros do conteúdo da página

const params = new URLSearchParams(window.location.search);
const gamePage = params.get("game");
...
const capitalGamePage = capitalizeFirst(gamePage);
...
const buttonNewReview = document.getElementById('reviewButton');
const divNewReview = document.getElementById('newReview');
...
document.addEventListener('click', (event) => {
    ...
    if (divNewReview.style.display === 'flex') {
        if (!divNewReview.contains(event.target)) {
            shadow.style.display = 'none';
            divNewReview.style.display = 'none';
        }
    }
});
...
buttonNewReview.addEventListener('click', (event) => {
    event.stopPropagation();
    if (username === 'null') {
        showCadastroBox();
    } else {
    divNewReview.style.display = 'flex';
    shadow.style.display = 'block';
    }
})
...
Object.keys(dataReviews).forEach(card => {
    if (dataReviews[card]['jogo'] == gamePage) {
...
// Carregar as informações de cada jogo

function calcRating() {
    let total = 0;
    let nota = 0;
    let ratingPrincipal = document.getElementById('rating');

    Object.keys(dataReviews).forEach(card => {
        if (dataReviews[card]['jogo'] == gamePage) {
            total += 1;
            nota += Number(dataReviews[card]['avaliacao']);
        }
    });
    ratingPrincipal.innerText = `${(nota / total).toFixed(2)} / 5.0`
}

document.addEventListener('DOMContentLoaded', () => {
    let imgPrincipal = document.getElementById('mainPicture');
    let namePrincipal = document.getElementById('gameName');
    let textPrincipal = document.getElementById('textGame');

    imgPrincipal.src = srcDict[gamePage];
    namePrincipal.innerText = `${capitalGamePage}`;
    textPrincipal.innerText = JSON.parse(localStorage.getItem("gameInfos"))[gamePage];
    calcRating();
})

// Criando uma nova review com dados enviados do usuário

document.getElementById('nameReview').innerText = `${capitalGamePage}`;

document.getElementById('formNewReview').addEventListener('submit', (event) => {
    event.preventDefault();
    let avaliacaoReview = document.getElementById('avaliacaoReview').value;
    let textoReview = document.getElementById('textReview').value;

    criaReview(gamePage, username, avaliacaoReview, textoReview, cardsReview);
    let idxNewReview = Math.max(...Object.keys(dataReviews).map(Number)) + 1;
    dataReviews[`0${String(idxNewReview)}`] = { 
        "user": username,
        "jogo": gamePage,
    ...
    calcRating();
    Swal.fire({
            title: 'Sua review foi publicada!',
            icon: "success",
            draggable: true,
            confirmButtonText: 'Fechar',
            background: '#0b0a0a',
